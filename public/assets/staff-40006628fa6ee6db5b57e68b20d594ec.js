...
(function() {
  ...
  $(document).on('turbolinks:load', function() {
    if ($('#number-of-unprocessed-messages').length) {
      return window.setInterval(update_number_of_unprocessed_messages, 1000 * 6);
    }
  });
...
(function() {
  ...
  window.named_paths['count_staff_messages'] = '/messages/count';

  window.named_paths['tag_staff_message'] = '/messages/:id/tag';

  window.path_for = function(name, ids) {
    var id, key, path;
    if (ids == null) {
      ids = {};
    }
